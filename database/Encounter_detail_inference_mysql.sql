...
(
    ...
    concept_type bigint(20) not null,
...
join concept_map m on m.event_type = 1008 and m.concept_type = 8038 and lower(concat(e.fhir_class, '|', e.fhir_type)) = m.source_term
set patient_status_concept_id = m.concept_id;
/*
select distinct lower(e.fhir_class || '|' || e.fhir_type) as class_type
...
update encounter_detail ed
join encounter_raw e on e.id = ed.id
join concept_map m on m.event_type = 1008 and m.concept_type = 8002 and lower(concat(e.fhir_adt_message_code,'|',ifnull(ed.patient_status_concept_id,'null'))) like m.source_term 
set ed.administrative_action_concept_id = m.concept_id;
/*
...
update encounter_detail ed
join encounter_raw e on e.id = ed.id
join concept_map m on m.event_type = 1008 and m.concept_type = 8041 and m.source_term = e.fhir_status
set completion_status_concept_id = m.concept_id;
/*
select distinct e.fhir_status
from encounter_raw e
join encounter_detail ed on ed.id = e.id
...
update encounter_detail ed
join encounter_raw e on e.id = ed.id
join concept_map m on m.event_type = 1008 and m.concept_type = 8000 and m.source_term = e.fhir_original_term
set ed.healthcare_service_type_concept_id = m.concept_id;
/*
select distinct e.fhir_original_term
from encounter_raw e
join encounter_detail ed on ed.id = e.id
